...
def main(verbose=False):
    ...
    parser.add_argument('-s', type=str, default=None, help='.npy file containing selection coefficients')
    ...
    if arg_list.covAtEachTime:
        np.savez_compressed(f, nVec=nVec, sVec=sVec, traj=traj, selections=selection, cov=totalCov, mu=mu, times=times, covAtEachTime=covAtEachTime)
    else:
        np.savez_compressed(f, nVec=nVec, sVec=sVec, traj=traj, selections=selection, cov=totalCov, mu=mu, times=times)
    f.close()
